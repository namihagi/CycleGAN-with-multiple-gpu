...
class Model(object):
    def __init__(self, sess, args):
        ...
        self.B_max_range = args.B_max_range
            ...
            for epoch in bar_epoch:
                ...
                with tqdm(range(max_iter), leave=False) as bar_iter:
                    for idx in bar_iter:
                        ...
                        # load data
                        A_feature, A_filename = self.sess.run(A_next_el)
                        B_feature, B_filename = self.sess.run(B_next_el)
                        ...
                        fake_A_sample, fake_B_sample, _, G_sum = \
                            self.sess.run([self.fake_A, self.fake_B, self.G_optim, self.G_sum],
                                          feed_dict={self.real_A: A_feature, self.real_B: B_feature, self.lr_ph: lr})
                        ...
                        _, D_sum = self.sess.run([self.D_optim, self.D_sum],
                                                 feed_dict={self.real_A: A_feature,
                                                            self.real_B: B_feature,
    ...
    def test(self):
        ...
        direction = {
            self.train_A_path: {
                'feature_dir': 'train/A2B',
                'placeholder': self.real_A,
                'output': self.fake_B,
            },
            self.train_B_path: {
                'feature_dir': 'train/B2A',
                'placeholder': self.real_B,
                'output': self.fake_A
            },
            self.test_A_path: {
                'feature_dir': 'test/A2B',
                'placeholder': self.real_A,
                'output': self.fake_B
            },
            self.test_B_path: {
                'feature_dir': 'test/B2A',
                'placeholder': self.real_B,
                'output': self.fake_A
            }
        }
        ...
        for target_dir in direction.keys():
            ...
            output_feature_dir = os.path.join(self.result_dir, setting['feature_dir'])
            makedirs(output_feature_dir)

            for idx in tqdm(range(file_num)):
                # load data
                feature, filename = self.sess.run(next_el)
                filename = filename[0].decode()

                generated_feature = self.sess.run(setting['output'], feed_dict={setting['placeholder']: feature})
                assert generated_feature[0].shape[0] == 32 and generated_feature[0].shape[2] == 128
                # generated_feature range (-1.0, 1.0)

                if 'A2B' in setting['feature_dir']:
                    generated_feature = (generated_feature + 1.0) * self.B_max_range / 2.0

                np.save(os.path.join(output_feature_dir, filename.replace('.jpg', '.npy')), generated_feature)
    ...
    def get_input_fn(self, dataset_path, max_range=1.0, is_training=True):
        filenames = os.listdir(dataset_path)
        filenames = [n for n in filenames if n.endswith('.tfrecords')]
        filenames = [os.path.join(dataset_path, n) for n in sorted(filenames)]

        with tf.device('/cpu:0'), tf.name_scope('input_pipeline'):
            pipeline = Pipeline(filenames, is_training=is_training, max_range=max_range,
