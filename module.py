...
class generator_resnet:
    ...
    def __call__(self, input, reuse=False):
        with tf.variable_scope(self.name):
            ...
            c1 = tf.nn.relu(instance_norm(conv2d(c0, c, 7, 1, padding='VALID', name='g_e1_c'), 'g_e1_bn'))  # 1024
            c2 = tf.nn.relu(instance_norm(conv2d(c1, c * 2, 3, 2, name='g_e2_c'), 'g_e2_bn'))               # 512
            c3 = tf.nn.relu(instance_norm(conv2d(c2, c * 4, 3, 2, name='g_e3_c'), 'g_e3_bn'))               # 256
            c4 = tf.nn.relu(instance_norm(conv2d(c3, c * 4, 3, 2, name='g_e4_c'), 'g_e4_bn'))               # 128
            # define G network with 9 resnet blocks
            r1 = residule_block(c4, c * 4, name='g_r1')
            ...
            d0 = deconv2d(r9, c * 4, 3, 2, name='g_d0_dc')
            d0 = tf.nn.relu(instance_norm(d0, 'g_d0_bn'))
            d1 = deconv2d(d0, c * 2, 3, 2, name='g_d1_dc')
            d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn'))
            d2 = deconv2d(d1, c, 3, 2, name='g_d2_dc')
...
class discriminator:
    ...
    def __call__(self, feature, reuse=False):
        with tf.variable_scope(self.name):
            ...
            # h0 is (512 x 512 x 64)
            h1 = lrelu(instance_norm(conv2d(h0, self.hidden_dim * 2, name='d_h1_conv'), 'd_bn1'))
            # h1 is (256 x 256 x self.hidden_dim*2)
            h2 = lrelu(instance_norm(conv2d(h1, self.hidden_dim * 4, name='d_h2_conv'), 'd_bn2'))
            # h2 is (128 x 128 x self.hidden_dim*4)
            h3 = lrelu(instance_norm(conv2d(h2, self.hidden_dim * 8, name='d_h3_conv'), 'd_bn3'))
            # h3 is (64 x 64 x self.hidden_dim*8)
            h4 = lrelu(instance_norm(conv2d(h3, self.hidden_dim * 8, s=1, name='d_h3_conv'), 'd_bn3'))
            # h4 is (64 x 64 x self.hidden_dim*8)
            h5 = conv2d(h4, 1, s=1, name='d_h3_pred')
            # h5 is (64 x 64 x 1)
        return tf.identity(h5, name=self.name+'_output')
